...
void main(int iActType, int bLawChaosAxis)
{
    ...
    if (bLawChaosAxis == 0) 
	{
		nAdjustment = CSLGoodEvilAxisAdjustment(oPC, iActType);
		CSLMessage_PrettyMessage("Alignment shifted on Good-Evil Axis by " + IntToString(nAdjustment));
		CSLMessage_PrettyMessage("Overall Alignment on this axis = " + IntToString(GetGoodEvilValue(oPC)));
	}
    else 
	{
		nAdjustment = CSLLawChaosAxisAdjustment(oPC, iActType);
		CSLMessage_PrettyMessage("Alignment shifted on Law-Chaos Axis by " + IntToString(nAdjustment));
		CSLMessage_PrettyMessage("Overall Alignment on this axis = " + IntToString(GetLawChaosValue(oPC)));
	}
}
