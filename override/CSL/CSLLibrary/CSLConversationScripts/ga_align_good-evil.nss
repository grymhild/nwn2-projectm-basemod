...
void main(int nActType, int nAlignmentShiftRule)
{
	...
	int nAdjustment	= CSLGetGoodEvilActAdjustment(nActType);
	...
	switch (nAlignmentShiftRule)
	{
		...
		case 1:
		{
			...
			{
				nNewAdjustment = CSLGetNeutralAdjustment(nAlignmentValue, nAdjustment);
			}
			break;
		}
		case 2:
	...
	CSLAdjustAlignmentGoodEvil(oPC, nNewAdjustment);
	
	CSLMessage_PrettyMessage("Alignment shifted on Good-Evil Axis by " + IntToString(nNewAdjustment));
