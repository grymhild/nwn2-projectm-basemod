...
/*
	Does an effect on the specified target.  This function takes 5 params:
		...
		string sTarget - Uses the standard CSLGetTarget() function - default is the PC Speaker
...
void main(string sEffect, string sParams, string sDuration, int iVisualEffect, string sTarget)
{
	...
	object oTarget = CSLGetTarget(sTarget, sTargetDefault);	
...
void DoEffect(string sEffect, string sParams, string sDuration, int iVisualEffect, object oTarget, string sDelim=",")
{
	int iDurationType = CSLGetDurationType(CSLNth_GetNthElement(sDuration, 1));
	float fDuration = 0.0f;
	if (iDurationType == DURATION_TYPE_TEMPORARY)
	{
		fDuration = CSLNth_GetNthElementFloat(sDuration, 2); // is 1 based, changed from 1 to 2 since old function was 0 based
	}
	...
	if(sEffect == "abilitydecrease"){
	    eEffect = EffectAbilityDecrease(CSLNth_GetNthElementInt(sParams, 1, sDelim), CSLNth_GetNthElementInt(sParams, 2, sDelim));
		iVis = VFX_IMP_REDUCE_ABILITY_SCORE;
	} else		
	if(sEffect == "abilityincrease"){
	    eEffect = EffectAbilityIncrease(CSLNth_GetNthElementInt(sParams, 1, sDelim), CSLNth_GetNthElementInt(sParams, 2, sDelim));
	...
	if(sEffect == "damage"){
	    eEffect = EffectDamage(CSLNth_GetNthElementInt(sParams, 1, sDelim), CSLNth_GetNthElementInt(sParams, 2, sDelim), CSLNth_GetNthElementInt(sParams, 3, sDelim));
	...
	if(sEffect == "disease"){
		eEffect = SupernaturalEffect(EffectDisease(CSLNth_GetNthElementInt(sParams, 1, sDelim)));
		iVis = 	VFX_IMP_DEATH;
	} else
	if(sEffect == "haste"){
		eEffect = EffectHaste();
	...
	if(sEffect == "heal"){
		eEffect = EffectHeal(CSLNth_GetNthElementInt(sParams, 1, sDelim));
	...
	if(sEffect == "poison"){
		eEffect = EffectPoison(CSLNth_GetNthElementInt(sParams, 1, sDelim));
	...
	if(sEffect == "visual"){
		eEffect = EffectVisualEffect(CSLNth_GetNthElementInt(sParams, 1, sDelim));
	} else
	{
