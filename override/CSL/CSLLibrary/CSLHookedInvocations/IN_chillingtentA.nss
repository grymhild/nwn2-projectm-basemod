...
void main()
{
	...
	if ( CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oCaster) )
	{
		...
		if((GetCreatureSize(oTarget)>CREATURE_SIZE_SMALL)&&(GetCreatureSize(oTarget)<=CREATURE_SIZE_HUGE)&&!CSLGetIsIncorporeal( oTarget )&&!GetIsImmune(oTarget,IMMUNITY_TYPE_MOVEMENT_SPEED_DECREASE,OBJECT_SELF))
		{
				...
				if( CSLGrappleCheck( oCaster, oTarget, HkGetSpellPower(oCaster, 30, CLASS_TYPE_WARLOCK) ) ) // only cast by warlocks
				{
					iDamage = d6() + 4;
...
/*
	...
	int nHits;
	float fDelay;
	//int iBonus = HkGetWarlockBonus(oCreator);
	effect eDam;
	if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oCreator)) {
		//Fire cast spell at event for the specified target
		SignalEvent(oTarget, EventSpellCastAt( oCreator, SPELL_I_CHILLING_TENTACLES));
		
        DelayCommand(0.1, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSlow, oTarget, RoundsToSeconds(1)));
		
		
		iDamage = 0;
		for (nHits = d4(); nHits>0; nHits--) {
			if (RunRollToHit(oTarget, oCreator)) { //ROLL TO HIT TARGET
		...
		int iSaveDC = GetInvocationSaveDC(oCreator); //GetSpellSaveDC();
