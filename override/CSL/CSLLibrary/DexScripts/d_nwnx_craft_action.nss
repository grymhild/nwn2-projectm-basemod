...
void main(int iAction, int iAvtValue)
{
   ...
   {
      XPCraft_Debug( oPC, "Cannot craft while polymorphed.");
   ...
   if (oCrafting!=OBJECT_INVALID && oCrafting!=oItemToCraft)
   {
      //SDB_LogMsg("CRAFTEQ", "Item not equipped: " + GetName(oCrafting) + " (" + IntToString(GetGoldPieceValue(oCrafting)) + ") and " + GetName(oItemToCraft)+ " (" + IntToString(GetGoldPieceValue(oItemToCraft)) + ")", oPC);
      oItemToCraft=oCrafting;
   }
