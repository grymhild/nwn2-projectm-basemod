...
{	
	...
	int iSpellId = SPELL_EPIC_ANBLIZZ;
	...
	if (GetCanCastSpell(OBJECT_SELF, SPELL_EPIC_ANBLIZZ))
	{
		...
		DelayCommand(0.1,
			HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, eShake, lTarget));
		DelayCommand(0.3,
			HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, eExplode, lTarget));
		DelayCommand(0.6,
			HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, eExplode, lTarget));
...
void DoAnimationBit(location lTarget, object oCaster, int nCasterLvl)
{
	...
	int nH = nM;
	string sWight = "csl_sum_undead_wight";
	object oWight;
	object oTarget = GetFirstObjectInShape(SHAPE_SPHERE,
		RADIUS_SIZE_HUGE, lTarget);
	while (GetIsObjectValid(oTarget))
		...
		if (nX < 5)
		{
				...
				GetLocalInt(oTarget, "nAnBlizCheckMe") == TRUE)
			{
				...
				else
				{
					...
					CSLDetermineCombatRound( oWight );
