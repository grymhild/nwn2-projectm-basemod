/** @file
* @brief Include File for Intelligent Weapons
*
* 
* 
*
...
void   IWSetConversationCondition(object oPlayer,int nType, int iValue);
...
// * iLevel - The power level of the weapon - unused at the moment (1 = default)
void IWCreateIntelligentWeapon(object oWeapon, int iLevel =1 );
...
object IWSpawnInWeaponCreature(object oPlayer, object oWeapon, int bEquip = TRUE)
{
	...
	if (bEquip)
	{
			...
			HkApplyEffectToObject(DURATION_TYPE_PERMANENT,eInvis,oSummon);
...
/*   object oCreator =      GetLocalObject(oWeaponCreature,"CREATOR");
	...
	DelayCommand(0.3f,HkApplyEffectToObject(DURATION_TYPE_PERMANENT,eInvis,oWeaponCreature));
	...
	HkApplyEffectToObject(DURATION_TYPE_INSTANT,eDis,oWeaponCreature);
...
void IWSetConversationCondition(object oPlayer,int nType, int iValue)
{
	...
	SetLocalInt(oPlayer,"X2_L_INTWEAPON_CONV_NUMBER", iValue);
}
...
void IWPlayRandomHitQuote(object oPlayer, object oWeapon, object oTarget)
{
	...
	if (GetIsDead(oTarget) || CSLGetHasEffectType( oTarget, EFFECT_TYPE_PETRIFY ))
	{
			return;
	}
...
void IWCreateIntelligentWeapon(object oWeapon, int iLevel =1 )
{
	if (CSLItemGetIsIntelligentWeapon(oWeapon))
	{
		return ;
	}
	itemproperty ipOnHit = ItemPropertyOnHitCastSpell(135,iLevel);
	CSLSafeAddItemProperty(oWeapon, ipOnHit,0.0f,SC_IP_ADDPROP_POLICY_REPLACE_EXISTING,TRUE,FALSE);
	itemproperty ipTalkTo = ItemPropertyCastSpell(536,IP_CONST_CASTSPELL_NUMUSES_UNLIMITED_USE);
	CSLSafeAddItemProperty(oWeapon,ipTalkTo,0.0f,SC_IP_ADDPROP_POLICY_REPLACE_EXISTING,TRUE,FALSE);
}
...
void IWSetEnhancementAndDrainLevel(object oWeapon, int nAddition, int bRemove = FALSE)
{
	if (bRemove)
	{
			CSLRemoveMatchingItemProperties(oWeapon,ITEM_PROPERTY_VISUALEFFECT,DURATION_TYPE_TEMPORARY);
			CSLRemoveMatchingItemProperties(oWeapon,ITEM_PROPERTY_ENHANCEMENT_BONUS,DURATION_TYPE_TEMPORARY);
			CSLRemoveMatchingItemProperties(oWeapon,ITEM_PROPERTY_DECREASED_ABILITY_SCORE,DURATION_TYPE_TEMPORARY);
			return;
	}
	...
	CSLSafeAddItemProperty(oWeapon,ip3,fTime, SC_IP_ADDPROP_POLICY_IGNORE_EXISTING);
	CSLSafeAddItemProperty(oWeapon,ip2,fTime, SC_IP_ADDPROP_POLICY_REPLACE_EXISTING, FALSE,TRUE);
	CSLSafeAddItemProperty(oWeapon,ip,fTime, SC_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,FALSE);
