...
//                Jug_Debug(GetName(OBJECT_SELF) + " casting spell on self " + IntToString(nSpellID) + " " + Get2DAString("spells", "Label", nSpellID));
                ActionCastSpellAtObject(nSpellID, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            return TRUE;
        }
        return FALSE;
...
//        Jug_Debug(GetName(OBJECT_SELF) + " casting spell on target " + GetName(oTarget) + " " + IntToString(nSpellID) + " " + Get2DAString("spells", "Label", nSpellID));
        ActionCastSpellAtObject(nSpellID, oTarget, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
        return TRUE;
    }
    return FALSE;
}
...
void TryCastInstantLongDurationBuffSpellOnTarget(object oTarget)
{
    ...
    {
        if (!TryCastInstantBuffSpell(SPELL_GREATER_RESISTANCE, oTarget, FALSE))
        {		
		    TryCastInstantBuffSpell(SPELL_CONVICTION, oTarget, FALSE);
		}
    }
...
void TryCastInstantMediumDurationBuffSpellOnTarget(object oTarget)
{
    ...
    {
	    if (!TryCastInstantBuffSpell(SPELL_ANIMALISTIC_POWER, oTarget, FALSE))
	    {
...
//	SPELL_I_ENTROPIC_WARDING
    ...
    if (!TryCastInstantBuffSpell(SPELL_TRUE_SEEING, oTarget, FALSE))
	{
	    if (!TryCastInstantBuffSpell(SPELL_BLINDSIGHT, oTarget, FALSE))
		{
		    if (!TryCastInstantBuffSpell(SPELL_SEE_INVISIBILITY, oTarget, TRUE))
			{
			    TryCastInstantBuffSpell(SPELL_INVISIBILITY_PURGE, oTarget, TRUE);
			}
		}
	}
	//943	See_Invisibility (feat)
	
    TryCastInstantBuffSpell(SPELL_SPELL_RESISTANCE, oTarget, FALSE);
    TryCastInstantBuffSpell(SPELL_PROTECTION_FROM_SPELLS, oTarget, FALSE);
}
...
void HenchQuickCastBuffs(int bMediumDuration)
{
    ...
    {
        SCTryCastWarlockBuffSpells(TRUE);
    }
}
...
object HenchCreateNonDroppableItem(string sItemTemplate, int nStackSize=1, int checkIfExists=FALSE)
{
	...
	// workaround for item use bug
	if (GetItemCharges(oInvItem) <= 0)
	{
		SetItemCharges(oInvItem, 1);
	}
...
//	Jug_Debug(GetName(OBJECT_SELF) + " running initialization");
	if (DEBUGGING >= 10) { CSLDebug(  "hench_o0_initialize Start "+GetName(OBJECT_SELF) + " Action =" +IntToString(GetCurrentAction()), GetFirstPC() ); }

	HenchIdentifyWeapons();
	
	if (SCGetHenchOption(HENCH_OPTION_ENABLE_AUTO_BUFF) && (GetAssociateType(OBJECT_SELF) == ASSOCIATE_TYPE_NONE) && !CSLGetHasEffectTypeGroup(OBJECT_SELF,EFFECT_TYPE_SPELL_FAILURE,EFFECT_TYPE_SILENCE))
	{
		HenchQuickCastBuffs(SCGetHenchOption(HENCH_OPTION_ENABLE_AUTO_MEDIUM_BUFF));
	}
	
	if (SCGetHenchOption(HENCH_OPTION_ENABLE_EQUIPPED_ITEM_USE))
	{
		HenchAllowUseofEquippedItemSpells();
	}
	
	if (SCGetHenchOption(HENCH_OPTION_ENABLE_ITEM_CREATION) &&
		(GetRacialType(OBJECT_SELF) != RACIAL_TYPE_UNDEAD) &&
		(GetAbilityScore(OBJECT_SELF, ABILITY_INTELLIGENCE) >= 6) &&
		SCGetCreatureUseItems(OBJECT_SELF))
	{
		HenchCreateItemsToUse();
	}
	if (DEBUGGING >= 10) { CSLDebug(  "hench_o0_initialize End", GetFirstPC() ); }
}