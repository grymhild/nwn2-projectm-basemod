...
//	Jug_Debug("*****" + GetName(OBJECT_SELF) + " heartbeat action " + IntToString(GetCurrentAction()) + " time " + FloatToString(time));
	...
	int bDying = (GetIsDead(OBJECT_SELF) || SCGetIsHenchmanDying(OBJECT_SELF));
    ...
    int bBusy = CSLGetAssociateState(CSL_ASC_IS_BUSY);
	...
	{
		if (!SCGetHasPlayerQueuedAction(OBJECT_SELF) && SCHenchCheckHeartbeatCombat(FALSE))
		{
