...
void CheckEquipped(object oWeapon, object oReach)
{
	...
	{
		CSLWrapperSetWeaponVisibility(oPC, 0, 0);
		DelayCommand(6.0f, CSLWrapperSetWeaponVisibility(oPC, 1, 0));
		DelayCommand(6.0f, DeleteLocalInt(oPC, "BurningBrand"));
		TOBStrikeTrailEffect(oReach, 1, 4.67f); // Contains routing for weapon type and makes the fire vfx.
		...
		else
		{
			...
			DelayCommand(6.0f, TOBProtectedClearCombatOverrides(oPC)); // Safe to use because it takes a swift action to initiate a stance or another boost.
		}
	}
...
{	
	...
	if ( !HkSwiftActionIsActive(oPC) )
	{
		...
		/*	if (oWeapon != oReach)
			...
			TOBStrikeTrailEffect(oWeapon, 1, 4.67f); // Contains routing for weapon type and makes the fire vfx.
			...
			TOBRunSwiftAction(3, "B");
			TOBExpendManeuver(3, "B");
		}
