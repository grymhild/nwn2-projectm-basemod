...
void WolfClimbsMountain(object oTarget, location lTarget, float fTarget)
{
	...
	if (GetIsObjectValid(oTarget))
	{
		...
		if ((fDist <= fTarget) && (GetLocalInt(oToB, "WolfClimbsCheck") == 0))
		{
			...
			HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eAC, oPC, 1.0f);
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 184);
	...
	float fTarget = CSLGetGirth(oTarget) + 0.01f;

	CSLStrikeAttackSound(oWeapon, oTarget, nHit, 0.2f);
	TOBBasicAttackAnimation(oWeapon, nHit);
	DelayCommand(0.3f, TOBStrikeWeaponDamage(oWeapon, nHit, oTarget, d6(5)));
	TOBExpendManeuver(184, "STR");
