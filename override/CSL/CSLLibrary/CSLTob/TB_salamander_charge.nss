...
void RunWallHeartDamage(string sPC, int nDC, effect eHit)
{
	...
	if (GetLocalInt(oPC, "SalamanderChargeActive") == 1)
	{
		...
		while (n <= nLocations)
		{
			if ((GetLocalInt(oTarget, "SalWall_" + sPC) == 0) && (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oPC))) //Do damage once per round per character using it.
			{
				nDamage = HkGetSaveAdjustedDamage(SAVING_THROW_REFLEX,SAVING_THROW_METHOD_FORHALFDAMAGE,d6(6), oTarget, nDC, SAVING_THROW_TYPE_FIRE);
...
void RunEdgeHeartDamage(string sPC, int nDC, effect eHit, object oPC = OBJECT_SELF )
{
	object oPC = OBJECT_SELF;
	...
	if (GetLocalInt(oPC, "SalamanderChargeActive") == 1)
	{
		...
		while (n <= nLocations)
		{
			...
			if (fDist > fFive)
			{
				if ((GetLocalInt(oTarget, "SalEdge_" + sPC) == 0) && (GetLocalInt(oTarget, "SalWall_" + sPC) == 0) && (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oPC))) //Do damage once per round per character using it.
				{
					nDamage = HkGetSaveAdjustedDamage(SAVING_THROW_REFLEX,SAVING_THROW_METHOD_FORHALFDAMAGE,d6(3), oTarget, nDC, SAVING_THROW_TYPE_FIRE);
...
{	
	...
	HkApplyEffectToObject(DURATION_TYPE_PERMANENT, eSalamander, oPC);
	DeleteLocalInt(oPC, "SalamanderNumber");
