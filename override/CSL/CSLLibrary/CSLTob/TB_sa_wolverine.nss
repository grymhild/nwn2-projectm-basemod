...
void WolverineStance(object oPC = OBJECT_SELF, object oToB = OBJECT_INVALID, int iSpellId = -1, int iSerial = -1)
{

	// void LoopFunction( object oPC = OBJECT_SELF, object oToB = OBJECT_INVALID, int iSpellId = -1, int iSerial = -1 )
	// oPC, oToB, iSpellId, iSerial
	if ( !GetIsObjectValid( oPC ) || !CSLSerialRepeatCheck( oPC, "WOLVERINESTANCE", iSerial ) )
	{
	...
	if ( iSerial == -1 )
	{
		iSerial = CSLSerialGetCurrentValue( oPC, "WOLVERINESTANCE" );
	...
	if (hkStanceGetHasActive(oPC,187))
	{
		...
		DelayCommand(6.0f, WolverineStance(oPC, oToB, iSpellId, iSerial));
	}
}
