...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 100);
	...
	if (nTouch > 0)
	{
		...
		if (nMyRoll >= nFoeRoll)
		{
			...
			location lLand = CSLGetAheadLocation(oPC, fSpace + fDistMod + FeetToMeters(10.0f));
			...
			{
				float fGirth = CSLGetMeleeRange(oTarget);
				...
				{
					if ((oCheck != oTarget) && (CSLGetIsStandardHostileTarget(oCheck, oPC)))
					{
						oComet = oCheck;
						break;
					}
				...
				CSLThrowTarget(oTarget, fSpace + fDistMod + FeetToMeters(10.0f));
				...
				if (GetIsObjectValid(oComet))
				{
					...
					if (fCheck <= fGirth)
					{
						int nDC = TOBGetManeuverDC(nMyStr, 0, 14);
						int nReflex = HkSavingThrow(SAVING_THROW_REFLEX, oComet, nDC);
						...
						if (nReflex > 0)
						{
							...
							DelayCommand(1.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eComet, oComet));
						}
						else
						{
							int nComet;
							...
							DelayCommand(1.1f, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKnockdown, oComet, 3.0f));
							DelayCommand(1.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eComet, oComet));
						}
					}
				}
			}
