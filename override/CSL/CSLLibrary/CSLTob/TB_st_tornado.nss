...
void TornadoThrow(object oPC, object oToB, location lPC)
{
	...
	if ((GetLocalInt(oPC, "TornadoGate") == 0) && (GetIsObjectValid(oTarget)) && (GetDistanceBetween(oPC, oTarget) - CSLGetGirth(oTarget) <= fRange))
	{
		...
		if (nTouch > 0)
		{
			...
			if (nMyRoll >= nFoeRoll)
			{
				...
				location lLand = CSLGetRandomLocation(GetArea(oPC), oPC, fSpace + fDistMod + FeetToMeters(10.0f));
				...
				CSLThrowTarget(oTarget, fSpace + fDistMod + FeetToMeters(10.0f), FALSE, FALSE);
...
{	
	...
	float fx = TOBGetManeuverX(oPC, 115);
	float fy = TOBGetManeuverY(oPC, 115);
	float fz = TOBGetManeuverZ(oPC, 115);
	...
	HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eTornado, oPC, 6.0f);
