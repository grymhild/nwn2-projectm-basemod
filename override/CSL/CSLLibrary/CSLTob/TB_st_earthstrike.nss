...
{	
	...
	int nDC = TOBGetManeuverDC(nStr, 0, 18);
	int nd20 = d20(1);
	int nRoll = nParry + nd20;
	float fRange = FeetToMeters(20.0f) + CSLGetGirth(oPC); // Starts from the edge of the PC's square rather than the center.
	...
	DelayCommand(2.8f, HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lPC));
	...
	{
		if ((oPC != oFoe) && (CSLSpellsIsTarget(oFoe, SCSPELL_TARGET_STANDARDHOSTILE, oPC)))
		{
			nReflex = HkSavingThrow(SAVING_THROW_REFLEX, oFoe, nDC);
			...
			{
				DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKnockdown, oFoe, 3.0f));
			}
			...
			if (nAction == ACTION_CASTSPELL)
			{
				...
				if (nRoll >= nFoeRoll)
				{
					HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eFizzle, oFoe, 6.0f);
				}
			}
		}
