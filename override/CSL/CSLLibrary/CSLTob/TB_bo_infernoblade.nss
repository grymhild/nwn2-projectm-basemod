...
{	
	...
	if ( !HkSwiftActionIsActive(oPC) )
	{
		...
		int nNumber = d6(3) + TOBGetInitiatorLevel(oPC);
		int nDamage = CSLGetDamageBonusConstantFromNumber(nNumber); //Caps at 40 which isn't a problem until epic levels.
		...
		// Removed due to a weird bug that ocurred when the PC died that prevented the removal of the damage property.
			...
			TOBRunSwiftAction(16, "B");
			TOBExpendManeuver(16, "B");
		}
