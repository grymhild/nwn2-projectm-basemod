		...
		if ((GetIsObjectValid(oFoe)) && (!GetIsImmune(oFoe, IMMUNITY_TYPE_KNOCKDOWN)))
		{
			...
			if ((nMySize == nFoeSize) || (nMySize + 1 == nFoeSize) || (nMySize - 1 == nFoeSize))
			{
				...
				if (nRoll >= nFoeRoll)
				{
					...
					HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBoulder, oFoe, 3.0f);
...
{	
	...
	float fx = TOBGetManeuverX(oPC, 145);
	float fy = TOBGetManeuverY(oPC, 145);
	float fz = TOBGetManeuverZ(oPC, 145);
	...
	if ( !HkSwiftActionIsActive(oPC) )
	{
		...
		TOBRunSwiftAction(145, "B");
		TOBExpendManeuver(145, "B");
