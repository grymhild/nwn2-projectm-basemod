		...
		if ((oWeapon == OBJECT_INVALID) || (oWeapon == GetItemInSlot(INVENTORY_SLOT_ARMS, oPC)))
		{
			...
			while (GetIsObjectValid(oCircle))
			{
				if ((oCircle != oTarget) && (GetIsReactionTypeHostile(oCircle, oPC)))
				{
					...
					DelayCommand(0.7f, CSLPlayCustomAnimation_Void(oPC, "*kickcircle", 0));
					break;
				}
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 75);
	...
	TOBStrikeTrailEffect(oWeapon, 1, 4.66f);
	CSLPlayCustomAnimation_Void(oPC, "*2attack01", 0);
	DelayCommand(3.0f, CSLPlayCustomAnimation_Void(oPC, "*2attack02", 0));
	TOBExpendManeuver(75, "STR");
	...
	int nHit = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);

	TimeStandsStill(oWeapon, oTarget, nHit);

	if (nFlurry > 0)
	{
		int nBlows = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nFlurry > 1)
	{
		int nBlows1 = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nSnap > 0)
	{
		int nKick = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nLHandAttacks > 0)
	{
		int nLeft1 = TOBStrikeAttackRoll(oOffHand, oTarget, nPenalty);
		TimeStandsStill(oOffHand, oTarget, nLeft1, 0.1f);
	}

	if (nRHandAttacks > 1) // Two attacks.
	{
		int nHit2 = TOBStrikeAttackRoll(oWeapon, oTarget, -5 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nHit2, 0.5f);
	}

	if (nLHandAttacks > 1)
	{
		int nLeft2 = TOBStrikeAttackRoll(oOffHand, oTarget, -5 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nLeft2, 0.6f);
	}

	if (nRHandAttacks > 2) // Three attacks.
	{
		int nHit3 = TOBStrikeAttackRoll(oWeapon, oTarget, -10 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nHit3, 1.0f);
	}

	if (nLHandAttacks > 2)
	{
		int nLeft3 = TOBStrikeAttackRoll(oOffHand, oTarget, -10 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nLeft3, 1.1f);
	}

	if (nRHandAttacks > 3) // Four attacks.
	{
		int nHit4 = TOBStrikeAttackRoll(oWeapon, oTarget, -15 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nHit4, 1.5f);
	}

	if (nLHandAttacks > 3)
	{
		int nLeft4 = TOBStrikeAttackRoll(oOffHand, oTarget, -15 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nLeft4, 1.6f);
	}

	if (nRHandAttacks > 4) // Five attacks.
	{
		int nHit5 = TOBStrikeAttackRoll(oWeapon, oTarget, -20 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nHit5, 2.0f);
	}

	if (nLHandAttacks > 4)
	{
		int nLeft5 = TOBStrikeAttackRoll(oOffHand, oTarget, -20 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nLeft5, 2.1f);
	}

	if (nRHandAttacks > 5) // Six attacks.
	{
		int nHit6 = TOBStrikeAttackRoll(oWeapon, oTarget, -25 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nHit6, 2.5f);
	}

	if (nLHandAttacks > 5)
	{
		int nLeft6 = TOBStrikeAttackRoll(oOffHand, oTarget, -25 + nPenalty);
	...
	if (GetCurrentHitPoints(oTarget) < 1)
	{
		...
		if (GetDistanceBetween(oPC, oCreature) <= fMelee)
		{
			...
			CSLWatchOpponent(oTarget);
		}
	}

	int nRoll = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);

	TimeStandsStill(oWeapon, oTarget, nRoll, 2.6f);

	if (nFlurry > 0)
	{
		int nBlows2 = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nFlurry > 1)
	{
		int nBlows21 = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nSnap > 0)
	{
		int nKick2 = TOBStrikeAttackRoll(oWeapon, oTarget, nPenalty);
	...
	if (nLHandAttacks > 0)
	{
		int nOffHand1 = TOBStrikeAttackRoll(oOffHand, oTarget, nPenalty);
		TimeStandsStill(oOffHand, oTarget, nOffHand1, 2.7f);
	}

	if (nRHandAttacks > 1) // Two attacks.
	{
		int nRoll2 = TOBStrikeAttackRoll(oWeapon, oTarget, -5 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nRoll2, 3.2f);
	}

	if (nLHandAttacks > 1)
	{
		int nOffHand2 = TOBStrikeAttackRoll(oOffHand, oTarget, -5 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nOffHand2, 3.3f);
	}

	if (nRHandAttacks > 2) // Three attacks.
	{
		int nRoll3 = TOBStrikeAttackRoll(oWeapon, oTarget, -10 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nRoll3, 3.6f);
	}

	if (nLHandAttacks > 2)
	{
		int nOffHand3 = TOBStrikeAttackRoll(oOffHand, oTarget, -10 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nOffHand3, 3.7f);
	}

	if (nRHandAttacks > 3) // Four attacks.
	{
		int nRoll4 = TOBStrikeAttackRoll(oWeapon, oTarget, -15 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nRoll4, 4.2f);
	}

	if (nLHandAttacks > 3)
	{
		int nOffHand4 = TOBStrikeAttackRoll(oOffHand, oTarget, -15 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nOffHand4, 4.3f);
	}

	if (nRHandAttacks > 4) // Five attacks.
	{
		int nRoll5 = TOBStrikeAttackRoll(oWeapon, oTarget, -20 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nRoll5, 4.6f);
	}

	if (nLHandAttacks > 4)
	{
		int nOffHand5 = TOBStrikeAttackRoll(oOffHand, oTarget, -20 + nPenalty);
		TimeStandsStill(oOffHand, oTarget, nOffHand5, 4.7f);
	}

	if (nRHandAttacks > 5) // Six attacks.
	{
		int nRoll6 = TOBStrikeAttackRoll(oWeapon, oTarget, -25 + nPenalty);
		TimeStandsStill(oWeapon, oTarget, nRoll6, 5.2f);
	}

	if (nLHandAttacks > 5)
	{
		int nOffHand6 = TOBStrikeAttackRoll(oOffHand, oTarget, -25 + nPenalty);
