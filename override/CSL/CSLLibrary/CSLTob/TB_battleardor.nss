...
//	Warblade's int bonus is included in TOBStrikeAttackRoll.	//
//////////////////////////////////////////////////////////////
...
{
	struct tob_main_damage rDamage = TOBGenerateAttackEffect(oPC, oWeapon, oFoe);
...
void BattleArdor( object oPC = OBJECT_SELF, object oToB = OBJECT_INVALID, int iSpellId = -1, int iSerial = -1 )
{
	// void LoopFunction( object oPC = OBJECT_SELF, object oToB = OBJECT_INVALID, int iSpellId = -1, int iSerial = -1 )
	// oPC, oToB, iSpellId, iSerial
	if ( !GetIsObjectValid( oPC ) || !CSLSerialRepeatCheck( oPC, "BATTLEARDOR", iSerial ) )
	{
	...
	if ( iSerial == -1 )
	{
		iSerial = CSLSerialGetCurrentValue( oPC, "BATTLEARDOR" );
	...
	if (GetIsInCombat(oPC) && (GetCurrentAction(oPC) == ACTION_ATTACKOBJECT))
	{
		...
		if ((nInt > 0) && (GetDistanceBetween(oPC, oTarget) <= fRange) && (!GetIsImmune(oTarget, IMMUNITY_TYPE_CRITICAL_HIT)))
		{
			...
			int nRoll = TOBStrikeAttackRoll(oWeapon, oFoe, 0, FALSE, nInt);
			int nOffHand;
			...
			{
				nOffHand = TOBStrikeAttackRoll(oOffHand, oFoe, 0, FALSE, nInt);
			}
			else nOffHand = 0;
			...
			if (nOffHand == 2)
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage, oFoe);
			}
			...
			{
				int nHit2 = TOBStrikeAttackRoll(oWeapon, oTarget, -5, FALSE, nInt);

				if (nHit2 == 2)
				{
					effect eDamage2 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(1.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage2, oFoe));
				}
			}

			if ((nLHandAttacks > 1) && (d100(1) > 50))
			{
				int nLeft2 = TOBStrikeAttackRoll(oOffHand, oTarget, -5, FALSE, nInt);

				if (nLeft2 == 2)
				{
					effect eLHDamage2 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(1.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage2, oFoe));
				}
			}

			if ((nRHandAttacks > 2) && (d100(1) > 50)) // Three attacks.
			{
				int nHit3 = TOBStrikeAttackRoll(oWeapon, oTarget, -10, FALSE, nInt);

				if (nHit3 == 2)
				{
					effect eDamage3 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(2.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage3, oFoe));
				}
			}

			if ((nLHandAttacks > 2) && (d100(1) > 50))
			{
				int nLeft3 = TOBStrikeAttackRoll(oOffHand, oTarget, -10, FALSE, nInt);

				if (nLeft3 == 2)
				{
					effect eLHDamage3 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(2.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage3, oFoe));
				}
			}

			if ((nRHandAttacks > 3) && (d100(1) > 50)) // Four attacks.
			{
				int nHit4 = TOBStrikeAttackRoll(oWeapon, oTarget, -15, FALSE, nInt);

				if (nHit4 == 2)
				{
					effect eDamage4 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(3.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage4, oFoe));
				}
			}

			if ((nLHandAttacks > 3) && (d100(1) > 50))
			{
				int nLeft4 = TOBStrikeAttackRoll(oOffHand, oTarget, -15, FALSE, nInt);

				if (nLeft4 == 2)
				{
					effect eLHDamage4 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(3.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage4, oFoe));
				}
			}

			if ((nRHandAttacks > 4) && (d100(1) > 50)) // Five attacks.
			{
				int nHit5 = TOBStrikeAttackRoll(oWeapon, oTarget, -20, FALSE, nInt);

				if (nHit5 == 2)
				{
					effect eDamage5 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(4.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage5, oFoe));
				}
			}

			if ((nLHandAttacks > 4) && (d100(1) > 50))
			{
				int nLeft5 = TOBStrikeAttackRoll(oOffHand, oTarget, -20, FALSE, nInt);

				if (nLeft5 == 2)
				{
					effect eLHDamage5 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(4.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage5, oFoe));
				}
			}

			if ((nRHandAttacks > 5) && (d100(1) > 50)) // Six attacks.
			{
				int nHit6 = TOBStrikeAttackRoll(oWeapon, oTarget, -25, FALSE, nInt);

				if (nHit6 == 2)
				{
					effect eDamage6 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(5.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage6, oFoe));
				}
			}

			if ((nLHandAttacks > 5) && (d100(1) > 50))
			{
				int nLeft6 = TOBStrikeAttackRoll(oOffHand, oTarget, -25, FALSE, nInt);

				if (nLeft6 == 2)
				{
					effect eLHDamage6 = TallyDamage(oWeapon, oPC, oFoe);
					DelayCommand(5.1f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eLHDamage6, oFoe));
				}
			}

			HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLoop, oPC, 6.0f);
			DelayCommand(6.0f, BattleArdor(oPC, oToB, iSpellId, iSerial));
		}
		else
		{
			HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLoop, oPC, 6.0f);
	...
	else
	{
		...
		DelayCommand(1.0f, BattleArdor(oPC, oToB, iSpellId, iSerial));
	}
}

void CheckLoopEffect( object oPC, object oToB = OBJECT_INVALID )
{
	if(!TOBCheckRecursive(6528))
	{
