...
void StepOfTheWind( object oPC = OBJECT_SELF )
{
	if (hkStanceGetHasActive(oPC,113))
	{
		...
		if ((nTerrain >= SM_TYPE_CARPET) && (nTerrain < SM_TYPE_METAL))
		{
			...
			HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBonus, oPC, 1.0f);
		}
		...
		DelayCommand(1.0f, StepOfTheWind(oPC));
	}
}
