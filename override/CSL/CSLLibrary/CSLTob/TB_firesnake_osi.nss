...
void Firesnake(object oFiresnake, object oPC, int nDC, float fRange, effect eHit)
{
	...
	while (GetIsObjectValid(oCreature))
	{
		...
		{
			nFire = d6(6);
			nDamage = HkGetSaveAdjustedDamage(SAVING_THROW_REFLEX,SAVING_THROW_METHOD_FORHALFDAMAGE,nFire, oCreature, nDC, SAVING_THROW_TYPE_FIRE);
			...
			if (nDamage > 0)
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_INSTANT, eHit, oCreature);
				HkApplyEffectToObject(DURATION_TYPE_INSTANT, eFiresnake, oCreature);
...
{	
	...
	float fRange = CSLGetMeleeRange(oFiresnake);
