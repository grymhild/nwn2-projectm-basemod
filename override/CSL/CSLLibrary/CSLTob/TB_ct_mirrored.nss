...
{
	if (hkCounterGetHasActive(oPC,108))
	{
		...
		if (lTarget != GetLocalLocation(oPC, "MirroredFirst_loc"))
		{
			...
			if (lLast == lTarget)
			{
				if ((CSLLineOfEffect(oPC, lTarget, TRUE)) && (GetDistanceBetween(oPC, oTarget) - CSLGetGirth(oTarget) <= fRange))
				{
					TOBClearStrikes();
					ClearAllActions();
