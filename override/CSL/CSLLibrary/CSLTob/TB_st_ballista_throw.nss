...
void BallistaThrow(object oTarget, float fSpace)
{
	...
	float fGirth = CSLGetGirth(oTarget); //Assuming that the rules as intended mean that we're throwing the target through others and therefore the damage area is equal to its size.
	...
	if (GetIsLocationValid(lLine))
	{
		...
		while (GetIsObjectValid(oBallista))
		{
			if ((GetIsReactionTypeHostile(oBallista, oPC)) && (GetLocalInt(oBallista, "BallistaThrow") == 0))
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage, oBallista);
			}
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 98);
	...
	if (nTouch > 0)
	{
		...
		if (nMyRoll >= nFoeRoll)
		{
			...
			CSLThrowTarget(oTarget, fSpace + FeetToMeters(60.0f), TRUE); //CSLThrowTarget automatically adjusts distance if we can't go the full 60 feet.
