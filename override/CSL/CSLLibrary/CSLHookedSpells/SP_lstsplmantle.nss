...
void main()
{
	//scSpellMetaData = SCMeta_SP_lstsplmantle();
	//if (!X2PreSpellCastCode()) { return; }  // If code within the PreSpellCastHook (i.e. UMD) reports FALSE, do not run this spell
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	object oCaster = OBJECT_SELF;
	int iSpellId = SPELL_LEAST_SPELL_MANTLE;
	...
	int iAbsorb = HkApplyMetamagicVariableMods(d4()+4, 8);
	int iNumberLevels = 9;
	...
	CSLUnstackSpellEffects(oTarget, SPELL_LEAST_SPELL_MANTLE);
