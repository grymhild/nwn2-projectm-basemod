...
void main()
{	
	//scSpellMetaData = SCMeta_SP_enershldls_e();
	//if (!X2PreSpellCastCode()) { return; }  // If code within the PreSpellCastHook (i.e. UMD) reports FALSE, do not run this spell
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	object oCaster = OBJECT_SELF;
	int iSpellId = SPELL_Energized_Shield_E;
	...
	effect eDamShld = EffectDamageShield(0, DAMAGE_BONUS_2d4, DAMAGE_TYPE_ELECTRICAL);
	effect eDamRes = EffectDamageResistance(DAMAGE_TYPE_ELECTRICAL, 10, 0);	
