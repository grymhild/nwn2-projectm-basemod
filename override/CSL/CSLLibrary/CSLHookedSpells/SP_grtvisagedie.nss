...
void main()
{
	//scSpellMetaData = SCMeta_SP_grtvisagedie();
	//if (!X2PreSpellCastCode()) { return; }  // If code within the PreSpellCastHook (i.e. UMD) reports FALSE, do not run this spell
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	object oCaster = OBJECT_SELF;
	int iSpellId = SPELL_GREATER_VISAGE_OF_THE_DEITY;
...
//Declare major variables
	...
	{
		SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, 1041));
		if (GetAlignmentGoodEvil(oCaster) == ALIGNMENT_GOOD)
		{
			...
			HkUnstackApplyEffectToObject(DURATION_TYPE_TEMPORARY, eGood, oCaster, fDuration, HkGetSpellId() );
		...
		else if (GetAlignmentGoodEvil(oCaster) == ALIGNMENT_EVIL)
		{
			...
			HkUnstackApplyEffectToObject(DURATION_TYPE_TEMPORARY, eEvil, oCaster, fDuration, HkGetSpellId() );
		...
		else
		{
			...
			HkUnstackApplyEffectToObject(DURATION_TYPE_TEMPORARY, eNeut, oCaster, fDuration, HkGetSpellId() );
