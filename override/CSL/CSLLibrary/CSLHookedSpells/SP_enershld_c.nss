...
void main()
{	
	//scSpellMetaData = SCMeta_SP_enershldls_c();
	//if (!X2PreSpellCastCode()) { return; }  // If code within the PreSpellCastHook (i.e. UMD) reports FALSE, do not run this spell
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	object oCaster = OBJECT_SELF;
	int iSpellId = SPELL_Energized_Shield_C;
	...
	effect eDamShld = EffectDamageShield(0, DAMAGE_BONUS_2d4, DAMAGE_TYPE_COLD);
	effect eDamRes = EffectDamageResistance(DAMAGE_TYPE_COLD, 10, 0);	
