...
void main()
{
	//scSpellMetaData = SCMeta_FT_grenthunders();
	//Declare major variables
	int iAttributes =106632;
		...
		if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
		{
				...
				fDelay = GetDistanceBetweenLocations(lTarget, GetLocation(oTarget))/20;
				{
					...
					if(!HkSavingThrow(SAVING_THROW_FORT, oTarget, iSaveDC))
					{
							...
							DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDeaf, oTarget, HkApplyDurationCategory(iDuration, SC_DURCATEGORY_ROUNDS) ));
