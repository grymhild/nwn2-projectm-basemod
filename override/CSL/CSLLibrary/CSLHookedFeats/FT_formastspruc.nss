...
void main()
{
    //scSpellMetaData = SCMeta_FT_formastspruc();
    ...
    int iCasterLevel = GetLevelByClass(CLASS_FOREST_MASTER, OBJECT_SELF);
    float fDuration  = HkApplyMetamagicDurationMods( RoundsToSeconds( GetLevelByClass(CLASS_FOREST_MASTER, OBJECT_SELF) ) );
...
//		nDmgType = DAMAGE_TYPE_POSITIVE;
	...
	if ( iCasterLevel >= 15 ) 		nDmgResist = 9;
	else if ( iCasterLevel >= 12 ) 	nDmgResist = 6;
	else 							nDmgResist = 3;
