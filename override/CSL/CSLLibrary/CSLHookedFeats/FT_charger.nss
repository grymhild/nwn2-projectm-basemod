...
{
	int iValue = GetLocalInt(OBJECT_SELF, "X0_L_CHARGES_ELECTRIFIER");
	iValue = iValue / 1000;
...
void main()
{
	//scSpellMetaData = SCMeta_FT_charger();
	int iAttributes = SCMETA_ATTRIBUTES_MAGICAL;
	object oItem = HkGetSpellTarget();
	...
	if (GetIsObjectValid(oItem))
	{
			if (GetObjectType(oItem) == OBJECT_TYPE_ITEM)
			{
				...
				if (nId == 510 && GetPlotFlag(oItem) == FALSE) // decharge
				{
					...
					SetLocalInt(OBJECT_SELF, "X0_L_CHARGES_ELECTRIFIER", nPreviousValue + iValue);
					effect eVis = EffectVisualEffect(VFX_IMP_DISPEL);
					HkApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, OBJECT_SELF);
					//SpeakString(IntToString(nPreviousValue + iValue));
					DestroyObject(oItem);
				}
			}
	}
