...
void main()
{
	...
	int iSaveDC = 14 + GetAbilityModifier(ABILITY_CHARISMA, OBJECT_SELF);
	...
	HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eConeBlast, OBJECT_SELF, 2.0f);
	...
	while(GetIsObjectValid(oTarget))
	{
			...
			nStunDuration = d4(3);
			iDamageAmount = d6(3);
			eDamage = EffectDamage(iDamageAmount);
			...
			DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage, oTarget));
				...
				// already stunned
				if (!GetHasSpellEffect(GetSpellId(), oTarget) && WillSave(oTarget, iSaveDC)==SAVING_THROW_CHECK_FAILED)
				{
				DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eHit, oTarget));
				DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eMindblast, oTarget, RoundsToSeconds(nStunDuration)));
			}
		}
