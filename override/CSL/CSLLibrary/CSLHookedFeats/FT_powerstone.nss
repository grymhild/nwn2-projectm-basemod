...
void DoStorm(object oPC, int nD6Dice, int nCap, int iSpellId, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE)
{
...
void main()
{
	...
	else if(GetCampaignInt("dbItems", "Q3B_POWERSTONE_OFF") == 4)
	{
			...
			location lImpacLocation = GetLocation(oPC); //GetLocation(oCaster);
				...
				{
					{
							...
							if(!HkSavingThrow(SAVING_THROW_FORT, oTarget, 20))
							{
								nRand = d4(1) + 2;
								effect eKnockdown = EffectKnockdown();
								HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKnockdown, oTarget, RoundsToSeconds(nRand));
	...
	else if(GetCampaignInt("dbItems", "Q3B_POWERSTONE_OFF") == 5)
	{
			...
			effect eFNF = EffectVisualEffect(VFX_DUR_SPELL_BESTOW_CURSE);//VFX_FNF_LOS_EVIL_20);
			int bContinueLoop = FALSE; //Used to determine if we have a next valid target
			int iHD = d4(12); //Roll to see how many HD worth of creature will be killed
			int nCurrentHD;
			...
			oTarget = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, HkGetSpellTargetLocation());
			...
			while ((iHD > 0) && (bContinueLoop))
			{
				...
				while (GetIsObjectValid(oTarget))
				{
					...
					oTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, HkGetSpellTargetLocation());
				}
				//Check to make sure that oLowest has changed
				if(bContinueLoop == TRUE)
				{
					SignalEvent(oLowest, EventSpellCastAt(oPC, SPELL_CIRCLE_OF_DEATH, TRUE ));
					//Make a Fort Save versus death effects
					if(!HkSavingThrow(SAVING_THROW_FORT, oLowest, HkGetSpellSaveDC(), SAVING_THROW_TYPE_DEATH, oPC, fDelay))
					{
	...
	else if(GetCampaignInt("dbItems", "Q3B_POWERSTONE_OFF") == 6)
	{
			...
			while(oCreature != OBJECT_INVALID)
			{
				...
				{
					if (!/*Will Save*/ HkSavingThrow(SAVING_THROW_WILL, oCreature, 18))
					{
							HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, eWord, GetLocation(oCreature));
							HkApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oCreature);
							HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oCreature, 6.0);
					}
				}
				n++;
