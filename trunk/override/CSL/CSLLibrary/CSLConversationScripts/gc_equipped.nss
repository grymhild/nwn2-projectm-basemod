...
int StartingConditional(string sItem, string sTarget, int bExactMatch)
{
	...
	if ("$PARTY" == sTarget)
	{
		...
		return (CSLGetPartyMemberHasEquipped(sItem, oTarget, FALSE, bExactMatch));
	}
	else
	{
		// Find target, default PC Speaker to check equipped item
		oTarget = CSLGetTarget(sTarget, CSLTARGET_PC);
		return (CSLGetIsItemEquipped(sItem, oTarget, bExactMatch));
	}
}