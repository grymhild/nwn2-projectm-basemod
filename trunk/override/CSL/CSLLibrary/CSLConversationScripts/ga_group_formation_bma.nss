...
void main(string sGroupTarget, float fSpacing, int iMoveType)
{
	...
	string sGroupName = "";
	object oGroupMember = CSLGetTarget(sGroupTarget, CSLTARGET_PC_SPEAKER);
	
	// if this is a PC party, use GetPartyGroup()
	if (GetIsObjectValid(SCGetPCLeader(oGroupMember)))
		sGroupName = SCGetPartyGroup(oGroupMember);	// creates group for the party and returns name.
	else
		SCGetGroupName(oGroupMember);
	
	
	if (fSpacing == 0.0f)
		fSpacing=DEFAULT_SPACING;
	SCGroupSetBMAFormation(sGroupName, fSpacing);	// a staggered marching formation
	if (iMoveType != 0)
		SCGroupMoveToObject(sGroupName, oGroupMember, iMoveType);
