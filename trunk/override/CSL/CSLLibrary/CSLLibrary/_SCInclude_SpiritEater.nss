/** @file
* @brief Include File for Spirit Eater Curse
*
* 
* 
*
...
int GetUniqueIDofPC(object oPC)
{
	...
	if (nUniqueID == 0)
	{
		nUniqueID = CSLIncrementGlobalInt(VAR_UNIQUE_ID_COUNT, 1);
...
void SetEffect(object oObject, int bEnable, effect eEffect, int nEffectSubType=0)
{
	...
	if (bEnable)
	{
		if (!CSLGetHasEffectType(oObject, nEffectType, nEffectSubType))
		{
			eEffect = CSLApplyEffectSubType(eEffect, nEffectSubType);
	...
	else
	{
		if (CSLGetHasEffectType(oObject, nEffectType, nEffectSubType))
		{
			CSLRemoveEffectByType( oObject, nEffectType, nEffectSubType );
		}
...
int SpiritEaterFeatAdd(int nFeat)
{
	...
	int nFeatStrRef = StringToInt(Get2DAString("feat", "Feat", nFeat));
	...
	CSLGlobalList_Push(VAR_SE_FEAT_LIST, IntToString(nFeat), TRUE);
	//string sList = GetGlobalString(VAR_SE_FEAT_LIST);
	//sList = CSLListAppendUnique(sList, IntToString(nFeat));
...
void ApplySpiritEaterFeatList(object oPC)
{
	...
	int bCheckReq = FALSE;
	int nFeat;
	//struct sCSLStringTokenizer stFeatList = CSLGetStringTokenizer(sList, ",");
	CSLStringArrayCreate(sList);
	while ( CSLStringArrayCount( ) > 0 )
	{
		//stFeatList = CSLAdvanceToNextToken(stFeatList);
		nFeat = StringToInt( CSLStringArrayPop() );
...
int GetIsSoul(object oTarget)
{
	...
	if ( !GetIsSpirit(oTarget) && CSLGetIsHumanoid(oTarget) )
	{
		return TRUE;
	}
	return FALSE;
...
*/
	...
	{
		if(HkGetSpellId() == SPELLABILITY_SPIRIT_GORGE)
		{
...
void DoDevour(object oTarget, int bApplyCorruptionPenalty=TRUE, int nDevourVFX = VFX_HIT_SPELL_DEVOUR_SPIRIT, object oCaster = OBJECT_SELF, int bCutscene = FALSE)
{
	int iDamage, nDamagePercent;
	int nSpiritEnergyPercentBonus = nDEVOUR_BASE_ENERGY;

	int nTargetMaxHP = GetMaxHitPoints(oTarget);
	int nCurrentHP = GetCurrentHitPoints(oTarget);
	iDamage = (nTargetMaxHP * nDEVOUR_MAX_ENERGY) / 100; // Target takes nDEVOUR_MAX_ENERGY% of max hp in damage.
	if (iDamage < 1)
			iDamage = 1;
	...
	{
		DelayCommand(VFX_SE_HIT_CUTSCENE_DELAY, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBeam, oTarget, VFX_SE_HIT_DURATION));
	}

	effect eDam = EffectDamage(iDamage, DAMAGE_TYPE_NEGATIVE);

	// if enough to kill creature, then more bonus energy!
	if (nCurrentHP <= iDamage)
	{
		...
		if(!bCutscene)
		{
			...
			DelayCommand(VFX_SE_HIT_DELAY + VFX_SE_HIT_DURATION, HkApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDeath(TRUE,TRUE,TRUE),oTarget));
		}
	...
	else //not a killshot
	{
		...
		{
			//standard delay
			DelayCommand(VFX_SE_HIT_DELAY, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
		}
		else
		{
			DelayCommand(VFX_SE_HIT_CUTSCENE_DELAY, AssignCommand(oTarget,DelayedPlayCustomAnimation(oTarget, "mjr_conjureloop",TRUE)));
			DelayCommand(VFX_SE_HIT_CUTSCENE_DELAY + VFX_SE_HIT_DURATION, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
		}
	}

	// SpeakString(GetName(oCaster) + " is applying damgage of " + IntToString(iDamage) + " to " + GetName(oTarget));
		// apply effects
...
void DoBestowLifeForce()
{
	...
	while (GetIsObjectValid(oTarget))
	{
		...
		// (dead people fail CSLSpellsIsTarget() checks)
		...
		if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_ALLALLIES, oCaster))
		{
			...
			if (nTargetCurrentHP < nTargetMaxHP)
			{
				...
				SignalEvent(oTarget, EventSpellCastAt(oCaster, HkGetSpellId(), FALSE)); // not harmful
...
void DoDevourCasterVFX(int nCasterEffect, object oCaster)
{
	...
	//AssignCommand(oCaster, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eCasterVis, oCaster));
