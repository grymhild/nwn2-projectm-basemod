...
void main()
{
	...
	{
			iDuration = (GetLevelByClass(CLASS_TYPE_SHIFTER,OBJECT_SELF)/3) +1;
			nDamagePower = CSLGetDamagePowerConstantFromNumber(iDuration);
	}
			nReduction = iDuration /2;
	if(nReduction <10)
	{
		nReduction = 10;
	}
	...
	effect eDam = EffectDamageReduction(nReduction, nDamagePower , iDuration*10);
