...
void main()
{
	//scSpellMetaData = SCMeta_FT_conefire();
	...
	int iDC = 10 + (iHD/2);
	float fDelay;
	HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_SPELL_BURNING_HANDS), oCaster, 2.0);
	effect eDamage;
	...
	while(GetIsObjectValid(oTarget)) {
		if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oCaster)) {
			...
			iDamage = d6(iDice);
			iDamage = HkGetSaveAdjustedDamage(SAVING_THROW_REFLEX,SAVING_THROW_METHOD_FORHALFDAMAGE,iDamage, oTarget, iDC, SAVING_THROW_TYPE_FIRE);
			if (iDamage) {
