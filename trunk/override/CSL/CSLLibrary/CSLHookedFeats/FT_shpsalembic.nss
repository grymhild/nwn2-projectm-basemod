...
int DoDistillationSkillCheck(int iSkillReq, object oPC=OBJECT_SELF)
{
	...
	//HkApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_CRAFT_SELF), oPC);
	HkApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_CRAFT_SELF), GetLocation(oPC));
	//SuccessNotify(oPC);
	return TRUE;
}
...
void CreateListOfItemsMultipleTimes(int iNumTimes, string sItemTemplateList, object oTarget, int bIdentify=TRUE)
{
	...
	{
		CSLCreateItemsInInventoryBasedOnList(sItemTemplateList, oTarget);
	}
}
...
int ConvertEssences(object oEssence, object oCaster)
{
			...
			|| (sType == ESSENCE_TYPE2_POWER))
	{
			...
			int nNewGrade = nGrade-1;
			sItemTemplateList = CSLDelimList(GetEssenceResRef(nGroup, ELEMENT_AIR, nNewGrade),
...
void main()
{
	//scSpellMetaData = SCMeta_FT_shpsalembic();
	...
	// ============================================================
	if (iSpellId == SPELL_SHAPERS_ALEMBIC_DIVIDE)
	{
			//PrettyDebug("Divide essence");
			DivideEssences(oTarget, oCaster);
	}
	else if (iSpellId == SPELL_SHAPERS_ALEMBIC_COMBINE)
	{
			//PrettyDebug("Combine essence");
			CombineEssences(oTarget, oCaster);
	}
	else if (iSpellId == SPELL_SHAPERS_ALEMBIC_CONVERT)
	{
