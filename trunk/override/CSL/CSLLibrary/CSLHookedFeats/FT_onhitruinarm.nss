...
void main()
{
	...
	if (GetIsObjectValid(oItem))
	{
		...
		{
			int iDC = GetBaseAttackBonus(oSpellOrigin) + GetAbilityModifier(ABILITY_STRENGTH,oSpellOrigin) +  8   - GetItemACValue(oItem);
			if (iDC > 0)
			{
				if (!GetIsSkillSuccessful(oSpellTarget, SKILL_DISCIPLINE, iDC) )
				{
