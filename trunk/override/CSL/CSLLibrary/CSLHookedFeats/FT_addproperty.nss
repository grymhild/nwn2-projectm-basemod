...
/*
	...
	The 2da used is defined in _CSLCore_Items
	SC_IP_ADDRPOP_2DA
...
void main()
{
	...
	int nPropID = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"PropertyType",nRow));
	int iDuration = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"Duration",nRow));
	int nParam1 = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"Param1",nRow));
	int nParam2 = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"Param2",nRow));
	int nParam3 = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"Param3",nRow));
	int nParam4 = StringToInt(Get2DAString(SC_IP_ADDRPOP_2DA,"Param4",nRow));

	// read success text
	string sSuccess = Get2DAString(SC_IP_ADDRPOP_2DA,"SuccessText",nRow);
	...
	itemproperty ip = CSLGetItemPropertyByID(nPropID, nParam1, nParam2, nParam3, nParam4);
	...
	fDuration = IntToFloat(iDuration);
	}

	if ( GetIsItemPropertyValid(ip))
	{
		CSLSafeAddItemProperty(oTarget, ip, fDuration,SC_IP_ADDPROP_POLICY_KEEP_EXISTING, FALSE,TRUE);
		if (GetItemHasItemProperty(oTarget,nPropID))
		{
			FloatingTextStringOnCreature(sSuccess,oUser);
			effect eVis = EffectVisualEffect(VFX_IMP_BREACH);
			HkApplyEffectToObject(DURATION_TYPE_INSTANT,eVis,oUser);
		}
		else
		{
			FloatingTextStrRefOnCreature(83360,oUser);         //"Nothing happens"
			effect eFail = EffectVisualEffect(VFXSC_FNF_BURST_SMALL_SMOKEPUFF);
			HkApplyEffectToObject(DURATION_TYPE_INSTANT,eFail,oUser);
		}
	}
	else
	{
