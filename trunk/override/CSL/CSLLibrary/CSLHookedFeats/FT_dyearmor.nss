...
/*
	...
	- the IPWork container (see _CSLCore_Items)
...
void FinishDyeScript(object oPC, object oTarget, int bEquipped, int nSlot)
{
	...
	CSLRemoveAllItemProperties(oNew,DURATION_TYPE_TEMPORARY);
...
void main()
{
	...
	object oNew = CopyItem(oTarget, CSLItemGetCraftingWorkContainer());


	DestroyObject(oTarget);

	// Dye the armor
	oTarget = CSLDyeArmor(oNew,nColorType, nColor);
