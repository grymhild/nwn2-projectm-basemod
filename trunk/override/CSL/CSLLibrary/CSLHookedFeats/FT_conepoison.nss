...
void main()
{
	//scSpellMetaData = SCMeta_FT_conepoison();
	...
	while(GetIsObjectValid(oTarget))
	{
			if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
			{
				...
				SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, SPELLABILITY_CONE_POISON, TRUE));
				//Get the delay time
				fDelay = GetDistanceBetween(OBJECT_SELF, oTarget)/20;
				DelayCommand(fDelay, HkApplyEffectToObject(DURATION_TYPE_PERMANENT, eCone, oTarget));
			}
