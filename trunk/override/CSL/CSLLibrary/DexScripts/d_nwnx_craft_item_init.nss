...
void main(int bStore, int iInventorySlot, string sRoadMap)
{
	...
	if(sSubAction =="Va") //initialising the Variation 
	{
		XPCraft_InitVariationValues(oPC);
	}
	else if(sSubAction =="Ar") //initialising the ArmorVisualType 
	{
		XPCraft_InitArmorVisualTypeValues(oPC);
	}
	else if(sSubAction =="AC") //initialising an ArmorPart 
	{
		XPCraft_InitItemPartValues(oPC, sRoadMap + "|Accessory");
	}
	else if(sSubAction == "Mo") //initialising a Weapon or Shield ModelPart	
	{
