...
void main()
{
    ...
    if( !GetIsObjectValid( oPillow1 ) )
    {
        ...
        oPillow1 = CreateObject( OBJECT_TYPE_PLACEABLE, "nw_waypoint001", locPillow1 );
        oPillow2 = CreateObject( OBJECT_TYPE_PLACEABLE, "nw_waypoint001", locPillow2 );
        oPillow3 = CreateObject( OBJECT_TYPE_PLACEABLE, "nw_waypoint001", locPillow3 );
    ...
    int iDistance = 1;
    object oPillow = GetNearestObjectByTag( "nw_waypoint001", oPlayer, iDistance );
    ...
    while( GetIsObjectValid( oPillow ) || iCount < 3 )
    {
        ...
        if( oPillow == oPillow1 || oPillow == oPillow2 || oPillow == oPillow3 )
        {
            ...
            if( !GetIsObjectValid( GetSittingCreature( oPillow ) ) )
            {
               ...
               lLoc = Location(GetArea(OBJECT_SELF), GetPosition(OBJECT_SELF), GetFacing(OBJECT_SELF)+fFacing);
               AssignCommand(oPlayer, JumpToLocation(lLoc));
               PlayCustomAnimation(oPlayer, "sitidle", 1);
        ...
        oPillow = GetNearestObjectByTag( "nw_waypoint001", oPlayer, iDistance );
    }
}
