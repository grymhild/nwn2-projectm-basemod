...
// SCSetSpawnInCondition(int nCondition, int bValid = TRUE)
//
//
//      case EVENT_HEARTBEAT:   // 1001 - CSL_FLAG_HEARTBEAT_EVENT
//          break;
//
//      case EVENT_PERCEIVE:    // 1002 - CSL_FLAG_PERCIEVE_EVENT
//          break;
//
//      case EVENT_END_COMBAT_ROUND:    // 1003 - CSL_FLAG_END_COMBAT_ROUND_EVENT
//          break;
//
//      case EVENT_DIALOGUE:    // 1004 - CSL_FLAG_ON_DIALOGUE_EVENT
//          break;
//
//      case EVENT_ATTACKED:    // 1005 - CSL_FLAG_ATTACK_EVENT
//          break;
//
//      case EVENT_DAMAGED:     // 1006 - CSL_FLAG_DAMAGED_EVENT
//          break;
//
//      case EVENT_DISTURBED:   // 1008 - CSL_FLAG_DISTURBED_EVENT
//          break;
//
//      case EVENT_SPELL_CAST_AT:   // 1011 - CSL_FLAG_SPELL_CAST_AT_EVENT
//          break;
...
//  		not needed, just clears out some flags
//			if (CSLGetAssociateState(CSL_ASC_MODE_PUPPET))    //NLC 7/02/08. I am a puppet. I put nothing on the ActionQueue myself.
//            	break;
            SCHenchHandlePlayerControlChanged(OBJECT_SELF);
			if (!SCGetHasPlayerQueuedAction(OBJECT_SELF))
			{
            	SCHenchResetCombatRound();
			}
            SCClearEnemyLocation();
            DeleteLocalInt(OBJECT_SELF, "HenchCurHealCount");
            break;
        }

        case 20000: // 20000 + ACTION_MODE_DETECT
        {
            if (CSLGetAssociateState(CSL_ASC_MODE_PUPPET)==TRUE)    //DBR 8/03/06 I am a puppet. I put nothing on the ActionQueue myself.
                break;
...
void CheckCreature(object oCreature)
{
    ...
    if( CSLGetHasEffectType(oCreature,EFFECT_TYPE_FRIGHTENED ) )
    {
        if(CheckSpell(SPELL_REMOVE_FEAR, oCreature)) return;
    }
    if( CSLGetHasEffectType(oCreature,EFFECT_TYPE_PARALYZE ) )
    {
        if(CheckSpell(SPELL_REMOVE_PARALYSIS, oCreature)) return;
    }

    if( CSLGetHasEffectTypeGroup( oCreature, EFFECT_TYPE_CONFUSED, EFFECT_TYPE_FRIGHTENED, EFFECT_TYPE_PARALYZE, EFFECT_TYPE_DOMINATED, EFFECT_TYPE_DAZED, EFFECT_TYPE_STUNNED ) )
    {
        if(CheckSpell(SPELL_GREATER_RESTORATION, oCreature)) return;
        if(CheckSpell(SPELL_RESTORATION, oCreature)) return;
        if(HasDisablingMagicalEffect(oCreature) &&  GetLocalInt(OBJECT_SELF, "X2_HENCH_DO_NOT_DISPEL") == 0)
        {
...
void CheckForDisabledPartyMembers()
{
    ...
    if ( CSLGetHasEffectType( OBJECT_SELF, EFFECT_TYPE_POLYMORPH ) == TRUE )
    {
        return;
    }
...
void HandleRosterSpawnIn()
{
    ...
    if ( GetGlobalInt( CAMPAIGN_SWITCH_UNPLOT_ON_ROSTER_SPAWN ) == TRUE )
    {
        ...
        if ( SCGetIsObjectInParty( OBJECT_SELF ) == TRUE )
        {
