...
{	
	...
	if (GetIsObjectValid(oToB))
	{
		...
		if ((!GetIsInCombat(oPC)) && (GetLevelByClass(CLASS_TYPE_SWORDSAGE, oPC) >= nLevel))
		{
			...
			{
				nLevelCap = GetLocalInt(oToB, "LevelupGate" + IntToString(CLASS_TYPE_SWORDSAGE));
			}
			...
			TOBLoadLevelup(oPC, oToB, CLASS_TYPE_SWORDSAGE, nLearn, nRetrain, nStance, nLevelCap);
		}
