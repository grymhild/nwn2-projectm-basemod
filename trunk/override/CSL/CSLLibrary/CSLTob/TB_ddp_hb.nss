...
/*	string sToB = sName + "tob";
	...
	if (nHP != nMaxHp)
	{
		...
		HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage, oPC);
	}

	effect eHeal = EffectHeal(nMaxHp);

	HkApplyEffectToObject(DURATION_TYPE_INSTANT, eHeal, oDDP);
	HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, ePool, oPC, 5.5f); // Shorter than a round due to the random delay added to heartbeats.
