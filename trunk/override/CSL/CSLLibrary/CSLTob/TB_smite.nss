...
{	
	...
	if (nUses > 0)
	{
		...
		int nRoll = TOBStrikeAttackRoll(oWeapon, oFoe, nCHA);

		CSLStrikeAttackSound(oWeapon, oFoe, nRoll, 0.2f);
		TOBBasicAttackAnimation(oWeapon, nRoll);
		DelayCommand(0.3f, TOBStrikeWeaponDamage(oWeapon, nRoll, oFoe, nLevel));
