...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 146);
	...
	if (nMySize == nFoeSize || (nMySize - 1) == nFoeSize || (nMySize + 1) == nFoeSize) //Target must be one size category smaller, the same size or one larger than the player.
	{
		...
		if (nMyRoll >= nFoeRoll)
		{
			...
			location lMove = CSLGetAheadLocation(oPC, fMove);

			DelayCommand(0.5f, PlayVoiceChat(VOICE_CHAT_GATTACK3, oPC));
			HkApplyEffectToObject(DURATION_TYPE_INSTANT, eMinotuar, oTarget);
			CSLPlayCustomAnimation_Void(oPC, "shieldbash", 0);
			CSLStrikeAttackSound(OBJECT_INVALID, oTarget, 1, 1.4);
			DelayCommand(1.41f, JumpToLocation(lMove));
			DelayCommand(1.35f, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSkid, oTarget, 0.2f));
			...
			lBullrush = CSLGetAheadLocation(oPC, fMove + fDistance);
			...
			lBullrush = CSLGetAheadLocation(oPC, fMove + FeetToMeters(5.0f));
			DelayCommand(1.4f, AssignCommand(oTarget, JumpToLocation(lBullrush)));
		}
		else
		{
			SendMessageToPC(oPC, "<color=red>Your bull rush attempt failed!</color>");
			CSLPlayCustomAnimation_Void(oPC, "*knockdownB", 0);
		}
	}
