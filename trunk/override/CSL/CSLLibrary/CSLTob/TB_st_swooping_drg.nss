...
void SwoopingDragonStrike(object oWeapon, object oTarget, int nTaunt)
{
	...
	DelayCommand(1.53f, TOBStrikeWeaponDamage(oWeapon, nHit, oTarget, d6(10)));

	if (!GetIsImmune(oTarget, IMMUNITY_TYPE_STUN))
	{
		int nFort = HkSavingThrow(SAVING_THROW_FORT, oTarget, nTaunt);
		...
		if (nFort == 0)
		{
			...
			HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSwoop, oTarget, 6.0f);
		}
	}
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 183);
