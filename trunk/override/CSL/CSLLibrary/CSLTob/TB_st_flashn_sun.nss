...
void FlashingSun(object oWeapon, object oTarget, int nHit, float fDelay = 0.0f, int bAnimate = TRUE)
{
	...
	if (bAnimate == TRUE)
	{
		...
		DelayCommand(fDelay, TOBBasicAttackAnimation(oWeapon, nHit, TRUE));
	}
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 13);
	...
	int nHit = TOBStrikeAttackRoll(oWeapon, oTarget, -2 + nPenalty);
	int nHit1 = TOBStrikeAttackRoll(oWeapon, oTarget, -2 + nPenalty);
	...
	if (nFlurry > 0)
	{
		int nBlows = TOBStrikeAttackRoll(oWeapon, oTarget, -2 + nPenalty);
	...
	if (nFlurry > 1)
	{
		int nBlows1 = TOBStrikeAttackRoll(oWeapon, oTarget, -2 + nPenalty);
	...
	if (nSnap > 0)
	{
		int nKick = TOBStrikeAttackRoll(oWeapon, oTarget, -2 + nPenalty);
	...
	if (nLHandAttacks > 0)
	{
		int nLeft1 = TOBStrikeAttackRoll(oOffHand, oTarget, -2 + nPenalty);
		FlashingSun(oOffHand, oTarget, nLeft1, 0.1f, FALSE);
	}

	if (nRHandAttacks > 1) // Two attacks.
	{
		int nHit2 = TOBStrikeAttackRoll(oWeapon, oTarget, -7 + nPenalty);
		FlashingSun(oWeapon, oTarget, nHit2, 1.0f, FALSE);
	}

	if (nLHandAttacks > 1)
	{
		int nLeft2 = TOBStrikeAttackRoll(oOffHand, oTarget, -7 + nPenalty);
		FlashingSun(oOffHand, oTarget, nLeft2, 1.1f, FALSE);
	}

	if (nRHandAttacks > 2) // Three attacks.
	{
		int nHit3 = TOBStrikeAttackRoll(oWeapon, oTarget, -12 + nPenalty);
		FlashingSun(oWeapon, oTarget, nHit3, 2.0f);
	}

	if (nLHandAttacks > 2)
	{
		int nLeft3 = TOBStrikeAttackRoll(oOffHand, oTarget, -12 + nPenalty);
		FlashingSun(oOffHand, oTarget, nLeft3, 2.1f, FALSE);
	}

	if (nRHandAttacks > 3) // Four attacks.
	{
		int nHit4 = TOBStrikeAttackRoll(oWeapon, oTarget, -17 + nPenalty);
		FlashingSun(oWeapon, oTarget, nHit4, 3.0f, FALSE);
	}

	if (nLHandAttacks > 3)
	{
		int nLeft4 = TOBStrikeAttackRoll(oOffHand, oTarget, -17 + nPenalty);
		FlashingSun(oOffHand, oTarget, nLeft4, 3.1f, FALSE);
	}

	if (nRHandAttacks > 4) // Five attacks.
	{
		int nHit5 = TOBStrikeAttackRoll(oWeapon, oTarget, -22 + nPenalty);
		FlashingSun(oWeapon, oTarget, nHit5, 4.0f, FALSE);
	}

	if (nLHandAttacks > 4)
	{
		int nLeft5 = TOBStrikeAttackRoll(oOffHand, oTarget, -22 + nPenalty);
		FlashingSun(oOffHand, oTarget, nLeft5, 4.1f, FALSE);
	}

	if (nRHandAttacks > 5) // Six attacks.
	{
		int nHit6 = TOBStrikeAttackRoll(oWeapon, oTarget, -27 + nPenalty);
		FlashingSun(oWeapon, oTarget, nHit6, 5.0f);
	}

	if (nLHandAttacks > 5)
	{
		int nLeft6 = TOBStrikeAttackRoll(oOffHand, oTarget, -27 + nPenalty);
