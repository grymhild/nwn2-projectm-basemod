...
void GenerateFoes(object oPC, object oToB)
{
	...
	{
		if ((oFoe != oPC) && (CSLGetIsStandardHostileTarget(oFoe, oPC)))
		{
...
void FountainOfBlood(float fThirty)
{
	...
	nDC += TOBGetManeuverDC(nStr, 0, 14);
	...
	{
		if (CSLGetIsStandardHostileTarget(oTarget, oPC))
		{
			nWill = HkSavingThrow(SAVING_THROW_WILL, oTarget, nDC);

			if (nWill == 0)
			{
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eFountain, oTarget, 60.0f);
			}
		}
...
{	
	...
	if ( !HkSwiftActionIsActive(oPC) )
	{
		...
		TOBRunSwiftAction(171, "B");
		TOBExpendManeuver(171, "B");
