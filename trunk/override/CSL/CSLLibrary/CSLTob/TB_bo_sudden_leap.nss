...
{	
	...
	if ( !HkSwiftActionIsActive(oPC) )
	{
		...
		float fx = TOBGetManeuverX(oPC, 182);
		float fy = TOBGetManeuverY(oPC, 182);
		float fz = TOBGetManeuverZ(oPC, 182);
		vector vTarget = Vector(fx, fy, fz);
		location lTarget;

		lTarget = Location(GetArea(oPC), vTarget, GetFacing(oPC));

		if (!GetIsLocationValid(lTarget) || lTarget == lPC || !CSLLineOfEffect(oPC, lTarget, TRUE) || !LineOfSightVector(GetPositionFromLocation(lPC), vTarget))
		{
		...
		int nArmorPenalty = abs(StringToInt(Get2DAString("Get2DAString", "ACCHECK", nArmor)));
		...
		else
		{
			...
			DelayCommand(1.02f, CSLPlayCustomAnimation_Void(oPC, "leapdown", 0));
		}

		TOBRunSwiftAction(182, "B");
		TOBExpendManeuver(182, "B");
	}
}
