...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 126);
	...
	DelayCommand(0.3f, TOBStrikeWeaponDamage(oWeapon, nHit, oTarget, d6(15)));
	TOBExpendManeuver(126, "STR");
	...
	if ((nHit > 0) && (nNonLiving == 0) && (!GetIsImmune(oTarget, IMMUNITY_TYPE_CRITICAL_HIT)) && (!GetIsImmune(oTarget, IMMUNITY_TYPE_ABILITY_DECREASE)))
	{
		...
		if (nFort == 0)
		{
			...
			if (nd20 <= 7)
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpecial, oTarget, RoundsToSeconds(nDuration));
				HkApplyEffectToObject(DURATION_TYPE_PERMANENT, eLegs, oTarget);
			...
			else if ((nd20 > 7) && (nd20 < 15))
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eVis, oTarget, 2.0f);
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpecial, oTarget, RoundsToSeconds(nDuration));
				HkApplyEffectToObject(DURATION_TYPE_PERMANENT, eArms, oTarget);
			...
			else
			{
				...
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eVis, oTarget, 2.0f);
				HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpecial, oTarget, RoundsToSeconds(nDuration));
				HkApplyEffectToObject(DURATION_TYPE_PERMANENT, eHeart, oTarget);
				DelayCommand(0.4f, TOBDropDead(oTarget, ABILITY_DEXTERITY));
				DelayCommand(0.4f, TOBDropDead(oTarget, ABILITY_STRENGTH));
			}
		}
