...
//	Author: Drammel									//
//	Date: 9/18/2009									//
//	Title: TB_st_soaring_throw							//
//	Description: If you succeed in tripping your 	//
// foe, you throw him up to 20 feet away from you. //
// The target falls prone in the destination space //
// and takes 8d6 points of damage. For every point //
// by which you win the opposed check, you gain an //
// additional foot of throw distance. For example, //
// if you win by 10, you can place your foe in any //
// space within 30 feet of you. You must place the //
// target in an empty space. If you lack the 		//
// distance to throw your target into a clear 	//
// space, he falls prone in his current space.		//
//////////////////////////////////////////////////////
...
{	
	...
	object oTarget = TOBGetManeuverObject(oToB, 111);
	...
	if (nTouch > 0)
	{
		...
		if (nMyRoll >= nFoeRoll)
		{
			...
			location lLand = CSLGetAheadLocation(oPC, fSpace + fDistMod + FeetToMeters(20.0f));
			effect eDamage = EffectDamage(d6(8), DAMAGE_TYPE_BLUDGEONING);
			effect eKnockdown = EffectKnockdown();

			if (GetIsLocationValid(lLand))
			{
				CSLThrowTarget(oTarget, fSpace + fDistMod + FeetToMeters(20.0f));
				DelayCommand(1.0f, HkApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKnockdown, oTarget, 3.0f));
				DelayCommand(1.0f, HkApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage, oTarget));
			}
			else
			{
