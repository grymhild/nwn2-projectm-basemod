...
void main()
{
	...
	if ( GetIsPC( oTarget ) )
	{
		DisplayGuiScreen(oTarget,"SCREEN_EXAMINE_JOURNAL", FALSE, "_SCjournal_data.xml");
		// SetLocalGUIVariable(oDM,"SCREEN_EXAMINE_JOURNAL", 0, ObjectToString(oDM) ); //IntToString(ObjectToInt(oDM) )
		int iRandom = Random(20000)+1;
		SetLocalInt( oTarget, "CSL_DATACALLBACKJOURNAL", iRandom );
		SetLocalGUIVariable(oTarget,"SCREEN_EXAMINE_JOURNAL", 5, IntToString(iRandom) );
		DelayCommand( 2.0f, DeleteLocalInt( oTarget,"CSL_DATACALLBACKJOURNAL") );

		DelayCommand( 2.0f, SendMessageToPC( oDM, "Journal: "+GetLocalString( oTarget, "JOURNAL") ) );
	}
	else
	{
