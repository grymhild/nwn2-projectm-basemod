...
//	if (!GetIsDM(oPC)) return;
	...
	if (GetStringLeft(sValue,5)=="SLOT_")
		...
		if ((GetObjectType(oTarget)!= 1) && (GetObjectType(oTarget)!=2))
			   {
			   SendMessageToPC(oPC,"You can only store items or creatures");	
			   return;
			   }
		...
		SendMessageToPC(oPC,"Succesfully added");
		...
		for(i=1;i<21;i++)
			...
			if (sNameB!="")
				...
				SetGUIObjectText(oPC,SCREEN_DMFI_MNGRTOOL,"BUT_MNGR_"+sSlot,0,"Empty"); 
			}
		}
		else
