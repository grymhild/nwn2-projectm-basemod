...
void main()
{
	//scSpellMetaData = SCMeta_SP_msinflictser();
	//if (!X2PreSpellCastCode()) { return; }  // If code within the PreSpellCastHook (i.e. UMD) reports FALSE, do not run this spell
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	object oCaster = OBJECT_SELF;
	int iSpellId = SPELL_MASS_INFLICT_SERIOUS_WOUNDS;
	...
	effect eHeal;
	int iSpellPower = HkGetSpellPower( OBJECT_SELF, 35 );
	...
	while (GetIsObjectValid(oTarget))
	{
			...
			//Roll damage
			iDamage = d8(3) + iSpellPower;
			//Make metamagic checks
			iDamage = HkApplyMetamagicVariableMods(iDamage, 24 + iSpellPower);
