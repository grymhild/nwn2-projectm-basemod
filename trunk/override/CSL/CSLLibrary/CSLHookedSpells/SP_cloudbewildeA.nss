...
void main()
{
	//scSpellMetaData = SCMeta_SP_cloudbewilde(); //SPELL_CLOUD_OF_BEWILDERMENT;
	//--------------------------------------------------------------------------
	//Prep the spell
	//--------------------------------------------------------------------------
	...
	{
		SignalEvent(oTarget, EventSpellCastAt(oCreator, SPELL_CLOUD_OF_BEWILDERMENT));
		//if (!HkResistSpell(oCreator, oTarget))
		//{
			if (!HkSavingThrow(SAVING_THROW_FORT, oTarget, HkGetSpellSaveDC(), SAVING_THROW_TYPE_POISON, OBJECT_SELF, 0.0f, SAVING_THROW_RESULT_REMEMBER))
			{
