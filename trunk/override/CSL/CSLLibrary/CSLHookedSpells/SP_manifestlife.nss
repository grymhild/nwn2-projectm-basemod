...
{	
	...
	int iSpellId = SPELL_MANIFEST_LIFE;
	...
	if (CSLSpellsIsTarget(oTarget, SCSPELL_TARGET_STANDARDHOSTILE, oCaster) &&
			CSLGetIsLiving(oTarget) )
	{

		int iTouch = CSLTouchAttackMelee(oTarget);
		if (iTouch != TOUCH_ATTACK_RESULT_MISS )
		{
			if(HkResistSpell(oCaster, oTarget) == 0)
			{
				...
				effect eDamage = EffectDamage(iDamage, DAMAGE_TYPE_DIVINE);
				...
				if (GetHasFeat(FEAT_LAY_ON_HANDS, oCaster, TRUE))
				{
					if (!GetHasFeat(FEAT_LAY_ON_HANDS))
					{
						IncrementRemainingFeatUses(oCaster, FEAT_LAY_ON_HANDS);
					}
					else
					{
