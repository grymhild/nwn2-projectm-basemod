...
void main()
{
	...
	string sOverrideAnimalCompanionResRef = GetAnimalCompanionOverrideResRef(OBJECT_SELF);

	int nFam = GetFamiliarCreatureType(OBJECT_SELF);
	int nAC = GetAnimalCompanionCreatureType(OBJECT_SELF);

	h2_LogMessage(H2_LOG_DEBUG, "Summon Aniimal Companion: " + sOverrideAnimalCompanionResRef + "AC Type = " + IntToString(nAC));

	if (sOverrideAnimalCompanionResRef == "")
		//Yep thats it
		SummonAnimalCompanion();
	else
		SummonAnimalCompanion(OBJECT_SELF, sOverrideAnimalCompanionResRef);
